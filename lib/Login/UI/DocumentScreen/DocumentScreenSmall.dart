...
class _DocumentScreenSmallState extends State<DocumentScreenSmall> {
  ...
  void initState() {
    ...
    if (globalRespObj != null) {
      ...
      if (globalRespObj.response.errorCode == "0") {
        ...
        _documentBloc.docPan =
            docObj.pancard.contains('pancard') ? true : false;
        _documentBloc.docSignature =
            docObj.signature.contains('signature') ? true : false;
        _documentBloc.docBankProof =
            docObj.bankproof.contains('bankproof') ? true : false;
        _documentBloc.docAddressProof =
            docObj.addressproof.contains('addressproof') ? true : false;
        _documentBloc.docIncomeProof =
            docObj.incomeproof.contains('incomeproof') ? true : false;
        _documentBloc.docPhotograph =
            docObj.photograph.contains('photograph') ? true : false;
  ...
  previewPDFImage(String fileName) {
    newWindow.openNewTab(AppConfig().url + '/images/' + fileName);
  }
  ...
  openUploadedImage(int id, BuildContext context) {
    switch (id) {
      case 101:
        print('PAN Card :$id');
        var fileName = LoginRepository
            .loginDetailsResObjGlobal.response.data.message[0].pancard;
      ...
      case 102:
        print(' Signature :$id');
        var fileName = LoginRepository
            .loginDetailsResObjGlobal.response.data.message[0].signature;
        if (fileName.contains('.pdf') || fileName.contains('.PDF')) {
          previewPDFImage(fileName);
        } else {
      ...
      case 103:
        print('Bank Proof :$id');
        var fileName = LoginRepository
            .loginDetailsResObjGlobal.response.data.message[0].bankproof;
        if (fileName.contains('.pdf') || fileName.contains('.PDF')) {
          previewPDFImage(fileName);
        } else {
      ...
      case 104:
        ...
        var fileName = LoginRepository
            .loginDetailsResObjGlobal.response.data.message[0].addressproof;
        if (fileName.contains('.pdf') || fileName.contains('.PDF')) {
          previewPDFImage(fileName);
        } else {
      ...
      case 105:
        ...
        var fileName = LoginRepository
            .loginDetailsResObjGlobal.response.data.message[0].incomeproof;
        if (fileName.contains('.pdf') || fileName.contains('.PDF')) {
          previewPDFImage(fileName);
        } else {
      ...
      case 106:
        print('Photograph :$id');
        var fileName = LoginRepository
            .loginDetailsResObjGlobal.response.data.message[0].photograph;
        if (fileName.contains('.pdf') || fileName.contains('.PDF')) {
          previewPDFImage(fileName);
        } else {
...
//Useful for Web
  ...
  Future _pickImageMobile(int id, ImageSource source) async {
    try {
      ...
      final pickedFile = await _picker.pickImage(
          source: source, imageQuality: 50, maxWidth: 100, maxHeight: 100);
      setState(() {
  ...
  void customOnPressed(int id, String proofType) {
    ...
    switch (id) {
      case 101:
        ...
        selectImageFile('pancard', proofType, docObj.uniqueId,
            ['png', 'jpeg', 'PNG', 'JPEG', 'jpg', 'JPG']);
        // openChoiceBottomsheetImagePicker(id, 'pancard', proofType);
        break;
      case 102:
        print(' Signature :$id');
        selectImageFile('signature', proofType, docObj.uniqueId,
            ['png', 'jpeg', 'PNG', 'JPEG', 'jpg', 'JPG']);
      ...
      case 103:
        print('Bank Proof :$id');
        selectImageFile(
            'bankproof', proofType, docObj.uniqueId, ['pdf', 'PDF']);
        // openChoiceBottomsheetImagePicker(id, 'bankproof', proofType);
        break;
      case 104:
        print('Address Proof :$id');
        selectImageFile('addressproof', proofType, docObj.uniqueId,
            ['png', 'jpeg', 'PNG', 'JPEG', 'jpg', 'JPG']);
      ...
      case 105:
        print('Income Proof :$id');
        selectImageFile(
            'incomeproof', proofType, docObj.uniqueId, ['pdf', 'PDF']);
        // openChoiceBottomsheetImagePicker(id, 'incomeproof', proofType);
        break;
      case 106:
        print('Photograph :$id');
        selectImageFile('photograph', proofType, docObj.uniqueId,
            ['png', 'jpeg', 'PNG', 'JPEG', 'jpg', 'JPG']);
  ...
  Future<dynamic> openChoiceBottomsheetImagePicker(
      int id, String docType, String proofType) {
                  ...
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        items:
                            _documentBloc.addressProofList.map((String value) {
                          return DropdownMenuItem(
                            value: value,
                            child: Text(
                              value,
                                  ...
                                  child: Container(
                                    ...
                                    child: TextButton(
                                        ...
                                        onPressed: () async {
                                          ...
                                          String proofType = "";
                                          if (_documentBloc
                                              .addressProof.isNotEmpty) {
                                            switch (
                                                _documentBloc.addressProof) {
                                              case "Voter ID":
                                                proofType = "0";
                                                break;
                  ...
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        items:
                            _documentBloc.incomeProofList.map((String value) {
                          return DropdownMenuItem(
                            value: value,
                            child: Text(
                              value,
                                  ...
                                  child: Container(
                                    ...
                                    child: TextButton(
                                        ...
                                        onPressed: () async {
                                          print('Pressed');
                                          if (_documentBloc
                                              .incomeProof.isNotEmpty) {
                                            String proofType = "";
                                            switch (_documentBloc.incomeProof) {
                                              case "ITR Currenct FY-AY":
                                                proofType = "0";
                                                break;
                                              case "PASSBOOK COPY - 6 MONTHS":
                                                proofType = "1";
                                                break;
                                              case "BANK STATEMENT - 6 MONTHS":
                                                proofType = "2";
                                                break;
                                              case "FORM 16":
                                                proofType = "3";
  ...
  Future<void> selectImageFile(String docType, String proofType,
      String uniqueId, List<String> extensionAllowed) async {
    ...
    if (result != null) {
      ...
      if (docType.toString() == "bankproof" ||
          docType.toString() == "incomeproof") {
        if (objFile.name.contains('.pdf') || objFile.name.contains('.PDF')) {
          if (sizeInMb > 2) {
            ...
            AppConfig().dismissLoaderDialog(context); //
            _documentBloc.saveDocument(context, docType, "small", objFile,
                objFile.bytes, proofType, uniqueId);
          }
        } else {
          AppConfig().dismissLoaderDialog(context); //
        ...
        if (docType.toString() == "pancard" ||
            docType.toString() == "signature" ||
            docType.toString() == "addressproof" ||
            docType.toString() == "photograph") {
          if (objFile.name.contains('.png') ||
              objFile.name.contains('.jpeg') ||
              objFile.name.contains('.PNG') ||
              objFile.name.contains('.JPEG') ||
              objFile.name.contains('.jpg') ||
              objFile.name.contains('.JPG')) {
            if (sizeInMb > 2) {
              // This file is Longer the
              print('file is big');
              AppConfig().dismissLoaderDialog(context); //
              _documentBloc.showAlert(
                  context, 'Maximum upload file size: 2 MB');
            } else {
              print('file is pefect');
              ...
              final thumbnail = img.copyResize(image,
                  width: widthsize.toInt(), height: heightsize.toInt());
              ...
              AppConfig().dismissLoaderDialog(context); //
              _documentBloc.saveDocument(context, docType, "small", objFile,
                  filess, proofType, uniqueId);
            }
          } else {
            AppConfig().dismissLoaderDialog(context); // show loader
            _documentBloc.showAlert(context,
                "Only 'png','PNG' 'jpeg', 'JPEG' ,'jpg' ,'JPG' supported");
          }
        }
      }
    } else {
              ...
              onPressed: () {
                if (_documentBloc.docPan &&
                  ...
                  HomeScreenSmall.screensStreamSmall.sink
                      .add(Ekycscreenamesmall.ipvdetailscreen);
                } else {
